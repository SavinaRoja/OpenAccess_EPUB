...
class OPSFrontiers(OPSMeta):
    ...
    def make_heading_affiliations(self, receiving_node):
        ...
        for aff in author_affs:
            ...
            bold.text = str(label_number) + ' '
            label_number += 1
            if len(aff.institution) > 0:
                institution = aff.institution[0].node
                element_methods.append_all_below(aff_item, institution)
            if len(aff.country) > 0:
                country = aff.country[0].node
                element_methods.append_all_below(aff_item, country)
